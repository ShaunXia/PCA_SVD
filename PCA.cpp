...
gsl_matrix* CPCA::EMPCA_SVD(gsl_matrix* matrix, const int pcNum)
{
    const int iterNum = 20; /** the time to interate below */
    const int dim = matrix->size2; 
    ...
    /* step 6: contribute the eigenValues and eigenMatrix of the cov USE SVD */
    gsl_vector* eval = gsl_vector_calloc (cov->size1);
    gsl_matrix* evec = gsl_matrix_calloc(cov->size1,cov->size1);
	gsl_matrix* A = gsl_matrix_calloc(cov->size1, cov->size2);
...
int main()
{
	...
	int pcNum=100;
    gsl_matrix* C = gsl_matrix_alloc(dim,pcNum);
	gsl_matrix* C_SVD = gsl_matrix_alloc(dim,pcNum);
    for(int i=0;i<dim;++i)
    {
        for(int j=0;j<pcNum;++j)
        {
            u = gsl_rng_uniform(r)+1; //产生一个[0, 1)区间上的随机数
            gsl_matrix_set(C,i,j,u - 0.5);
			gsl_matrix_set(C_SVD,i,j,u - 0.5);
        }
    }
	//printf("Matrix is :\n");
	/*for(int i=0;i<dim;++i)
...
*/
	...
	printf("empca time is %f\n",totaltime);

	start=clock();
	CPCA::EMPCA_SVD(C_SVD,pcNum);
	finish=clock();
	totaltime=(double)(finish-start)/CLOCKS_PER_SEC;
/*
	printf("EMPCA_SVD Matrix is :\n");
	for(int i=0;i<dim;++i)
    {
        for(int j=0;j<pcNum;++j)
        {
            printf("%f ",gsl_matrix_get(C_SVD,i,j));
        }
		printf("\n");
    }
*/
	printf("empca_SVD time is %f\n",totaltime);
